...
@Component({
  selector: 'app-add-payment-out-cost-center',
  templateUrl: './add-payment-out-cost-center.component.html',
  styleUrls: ['./add-payment-out-cost-center.component.scss']
})
export class AddPaymentOutCostCenterComponent implements OnInit {
  ...
  ngAfterViewInit(): void {
    setTimeout(() => {    
    if (this.config.data?.paymentOutDetailCostCenters?.length) {
      console.log(this.config.data?.paymentOutDetailCostCenters,"this.config.data?.paymentOutDetailCostCentersthis.config.data?.paymentOutDetailCostCenters");
      
      this.allocationform.clear();
      this.config.data.paymentOutDetailCostCenters.forEach((element: any) => {
