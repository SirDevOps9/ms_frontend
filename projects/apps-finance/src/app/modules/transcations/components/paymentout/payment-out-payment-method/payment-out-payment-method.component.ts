...
@Component({
  selector: 'app-payment-out-payment-method',
  templateUrl: './payment-out-payment-method.component.html',
  styleUrls: ['./payment-out-payment-method.component.scss']
})
export class PaymentOutPaymentMethodComponent implements OnInit {
  addForm: FormGroup;
  ...
  ngOnInit() {
    if (this.config.data) {
      ...
      if (this.config.data?.paymentOutMethodDetail) {
        const paymentDetails = this.config.data.paymentOutMethodDetail;
        this.initializeForm();
        setTimeout(() => {
      ...
      }else  if (this.config.data?.paymentOutMethodDetails) {
        const paymentDetails = this.config.data.paymentOutMethodDetails;
        this.initializeForm();
        setTimeout(() => {
          this.addForm.patchValue({
            paymentMethodId: paymentDetails.paymentMethodId || this.paymentmethodId,
