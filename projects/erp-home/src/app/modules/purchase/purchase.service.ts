...
import { LoaderService, LanguageService, ToasterService, RouterService, PageInfo, PageInfoResult } from 'shared-lib';
import { VendorCategoryDto, AddVendorCategory, EditVendorCategoryDto, vendorDefinitionDto, AddVendorCommand, CityDto, CountryDto, CurrencyDto, TagDropDownDto } from './models';
import { CategoryDropdownDto } from '../general-setting/models';
...
export class PurchaseService {
  ...
  getVendorCategoryByID(id : number) {
    this.purchaseProxy.getVendorCategoryByID(id)
    .subscribe(res=>{
  ...
  getVendorCategoryDropdown() {
    this.purchaseProxy.getVendorCategoryDropdown()
    .subscribe(res=>{
      if(res) {
  ...
  getVendorDefinitionByID(id : number) {
    this.purchaseProxy.getVendorDefinitionByID(id)
    .subscribe(res=>{
  ...
  getChildrenAccountsDropDown() {
    this.purchaseProxy.getChildrenAccountsDropDown()
    .subscribe(res=>{
      if(res) {
  ...
  getpriceListDropDown() {
    this.purchaseProxy.getpriceListDropDown()
    .subscribe(res=>{
      if(res) {
  ...
  getpaymentTermsListDropDown() {
    this.purchaseProxy.getpaymentTermsListDropDown()
    .subscribe(res=>{
      if(res) {
