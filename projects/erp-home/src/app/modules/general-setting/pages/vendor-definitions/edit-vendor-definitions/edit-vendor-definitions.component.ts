  ...
  intitializeFormData(){
    ...
    this.generalSettingService.vendorDefinitionDataByIDObservable.subscribe((res ) => {
      if (res) {
        ...
        this.vendor= res;
        this.selectedVendorCategory = res.vendorCategory?.name ?? '';
        ...
        this.selectedCity = res.vendorAddress?.cityName ?? '';
        this.selectedPaymentTerm = res.vendorFinancial?.paymentTermName ?? '';
        this.selectedPriceList = res.vendorFinancial?.priceListName ?? '';
        this.selectedCurrency = res.vendorFinancial?.currencyName ?? '';
        this.selectedPayableAccount = res.vendorAccounting?.payableAccountName ?? '';
        this.selectedPurchaseAccount = res.vendorAccounting?.purchaseAccountName ?? '';
        this.selectedPurchaseReturnAccount = res.vendorAccounting?.purchaseReturnAccountName ?? '';
        this.selectedDiscountAccount = res.vendorAccounting?.discountAccountName ?? '';
      }
    });
  }
