...
export class EditFinancialCalendarComponent implements OnInit {
  constructor(private fb :FormBuilder , private generalSettingService: GeneralSettingService,private route : ActivatedRoute , private formsService: FormsService){
  ...
  this.generalSettingService.FinancialPeriodDataByIDObservable.subscribe((res : editFinancialCalndar)=>{
  ...
  onEdit() {
    ...
    this.generalSettingService.editFinancialPeriod({id:this.id,name:this.formGroup.get('name')?.value})
