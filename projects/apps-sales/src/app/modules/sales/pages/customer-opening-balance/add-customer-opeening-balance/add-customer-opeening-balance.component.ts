...
export class AddCustomerOpeeningBalanceComponent implements OnInit {
  ...
  //   console.log(event , "4444444444444")
  ...
  accountSelected(event: any, index: number) {
    const bankLine = this.items.at(index); // Get the FormGroup at the given index
...
const ref = this.dialog.open(CustomerOpeningBalanceDistributeComponent, {
  ...
  getLinesDropDown(id: number) {
    ...
    this.SalesService.LinesDropDownDataObservable.subscribe((res:any) => {
      ...
      console.log(this.LinesDropDown,"888888");
...
console.log("222222222222");
  ...
  //      this.items.patchValue = res.customerOpeningDetails;
  //     console.log(res ,"1111111111");
  ...
  //   this.SalesService.getCustomerOpeningBalance();
  //   this.SalesService.CustomerOpeningBalancelistObservable.subscribe((res: any) => {
  //     console.log(res, "Received Customer Opening Balance Data");
  
     
  
  //     // Patch the main formGroup with the initial data
  //     this.formGroup.patchValue({
  //       open: res.openingJournalEntryId,  // Assuming 'openingJournalEntryId' exists in the response
  //       open2: res.journalLineId,         // Assuming 'journalLineId' exists in the response
  //       name1: res.amount,
  //       name2: res.amountNature
  //     });
  
  //     // Iterate over the response and add FormGroups to the FormArray
  //     res.customerOpeningDetails.forEach((detail: any) => {
  //       const formGroup = this.createBankFormGroup();
  //       formGroup.patchValue(detail);
  //       this.customerForm.push(formGroup);
  //     });
  
  //     console.log(this.customerForm.value, "Patched FormArray Data");
  //   });
  // }
  getCustomerOpeningBalance() {
    this.SalesService.getCustomerOpeningBalance();
    this.SalesService.CustomerOpeningBalancelistObservable.subscribe((res: any) => {
      console.log(res, "Received Customer Opening Balance Data");
      // Ensure customerForm is initialized before processing
      if (!this.customerForm) {
        console.error("customerForm is undefined. Initializing...");
        this.customerForm = this.fb.array([]);
      } else {
        this.customerForm.clear();
      }
  
      // Check if res and its properties are valid before proceeding
      if (res) {
        this.formGroup.patchValue({
        ...
        //this.getCustomerDropDownByAccountId(5)
        setTimeout(() => {
          if(this.LinesDropDown.length!=0){
            this.LinesDropDown.forEach((element: any) => {
              if (element.id == res.openingBalanceJournalEntryLineId ) {
                ...
                //this.customerForm.controls['displayName'].setValue(element.code)
              }
            }); 
          }
        }, 500);
        ...
        if (res.customerOpeningDetails && Array.isArray(res.customerOpeningDetails)) {
          res.customerOpeningDetails.forEach((detail: any) => {
            const formGroup = this.createBankFormGroup();
  // this.accountSelected(detail.customerId,detail.customerId)
            // Check if the detail is valid before patching
            this.accountSelected(detail.customerId, this.customerForm.length - 1);
        ...
        console.error("Response is undefined or invalid.");
      }
